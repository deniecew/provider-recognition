...
name_filter <- commentdata %>%
  filter(npi_num == params$npi) %>%
  # Split by comma to a list-column, trim spaces, drop empties
  mutate(parts = str_split(provider_nm, ",")) %>%
  mutate(parts = map(parts, ~ .x %>% str_trim() %>% discard(~ .x == ""))) %>%
  # Build fullname = First + Last + optional credentials joined by ", "
  mutate(fullname = map_chr(parts, function(x) {
    n <- length(x)
    if (n == 0) return("")
    if (n == 1) return(x[1])
    last <- x[1]
    first <- x[n]
    creds <- if (n > 2) x[2:(n - 1)] else character(0)
    paste0(first, " ", last, if (length(creds)) paste0(" ", paste(creds, collapse = ", ")) else "")
  })) %>%
  distinct(fullname) %>%
  select(fullname)

program_filter <- providermatches %>%
  filter(npi_num == params$npi) %>%
  distinct(program)
...
first_or_na <- function(x) {
  x <- as.character(x)
  if (length(x) == 0 || all(is.na(x))) NA_character_ else x[1]
}

summary_row <- tibble(
  npi_num = as.character(params$npi),
 
  program = as.character(named_comments$program_nm[1]),
  
  provider_nm = {
    if ("fullname" %in% names(name_filter)) first_or_na(name_filter$fullname) else NA_character_
  },

  # earliest date among matched comments
  rec_date = {
    if (!is.null(named_comments) && "recdate" %in% names(named_comments) && nrow(named_comments) > 0) {
      suppressWarnings(min(named_comments$recdate, na.rm = TRUE))
    } else as.Date(NA)
  },

  shout_out = {
    if (!is.null(bigram_sentiment) && "response" %in% names(bigram_sentiment) && nrow(bigram_sentiment) > 0) {
      as.character(bigram_sentiment$response[1])
    } else NA_character_
  }
)
...
write_csv(summary_row, file = file.path("tmp", paste0(params$npi,"_",summary_row$rec_date, ".csv")))
...
`r if (n == 0) {
      ...
      paste0("Comment received ", summary_row$rec_date)
    }`
:::
